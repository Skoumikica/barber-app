    ...
    <div style={{ maxWidth: 480, margin: '0 auto', fontFamily: 'sans-serif', backgroundColor: theme.bg, minHeight: '100vh', paddingBottom: 100 }}>
      <div style={{ background: 'linear-gradient(135deg, #1e3a8a, #2563eb)', padding: '20px 20px 28px', borderBottomLeftRadius: 24, borderBottomRightRadius: 24 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <h2 style={{ color: 'white', fontSize: 20, fontWeight: 'bold', margin: 0 }}>
            {theme.jezik === 'sr' ? 'Moj Profil' : 'My Profile'}
          </h2>
        </div>
      </div>
      ...
      <div style={{ padding: 20 }}>
        ...
        {success && (
          <div style={{ backgroundColor: '#dcfce7', borderRadius: 10, padding: '10px 14px', marginBottom: 16, textAlign: 'center' }}>
            <p style={{ color: '#16a34a', fontSize: 14, fontWeight: 'bold', margin: 0 }}>
              {theme.jezik === 'sr' ? '✓ Profil uspešno sačuvan!' : '✓ Profile saved successfully!'}
            </p>
          </div>
        )}

        <div style={{ backgroundColor: theme.card, borderRadius: 16, padding: 20, marginBottom: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <h3 style={{ fontSize: 15, fontWeight: 'bold', marginBottom: 16, color: theme.text }}>
            {theme.jezik === 'sr' ? 'Lični podaci' : 'Personal info'}
          </h3>
          <p style={{ fontSize: 13, color: theme.subtext, marginBottom: 6 }}>{t.tvojeIme}</p>
          <input placeholder={t.imePrezime} value={ime} onChange={e => setIme(e.target.value)} style={inputStyle} />

          <p style={{ fontSize: 13, color: theme.subtext, marginBottom: 6 }}>{t.emailAdresa}</p>
          <input placeholder="Email" value={email} disabled style={{ ...inputStyle, opacity: 0.6, cursor: 'not-allowed' }} />
        </div>

        <div style={{ backgroundColor: theme.card, borderRadius: 16, padding: 20, marginBottom: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.08)' }}>
          <h3 style={{ fontSize: 15, fontWeight: 'bold', marginBottom: 16, color: theme.text }}>
            {theme.jezik === 'sr' ? 'Podaci salona' : 'Salon info'}
          </h3>
          <p style={{ fontSize: 13, color: theme.subtext, marginBottom: 6 }}>{t.nazivSalona}</p>
          <input placeholder={t.nazivSalona} value={salonNaziv} onChange={e => setSalonNaziv(e.target.value)} style={inputStyle} />

          <p style={{ fontSize: 13, color: theme.subtext, marginBottom: 6 }}>{t.adresaSalona}</p>
          <input placeholder={t.adresaSalona} value={adresa} onChange={e => setAdresa(e.target.value)} style={inputStyle} />

          <p style={{ fontSize: 13, color: theme.subtext, marginBottom: 6 }}>{t.brojTelefona}</p>
          <input placeholder={t.brojTelefona} value={telefon} onChange={e => setTelefon(e.target.value)} style={{ ...inputStyle, marginBottom: 0 }} />
        </div>
      </div>
      ...
      <div style={{ position: 'fixed', bottom: 0, left: '50%', transform: 'translateX(-50%)', width: '100%', maxWidth: 480, padding: '12px 20px', backgroundColor: theme.card, borderTop: `1px solid ${theme.border}`, boxSizing: 'border-box' }}>
        <button onClick={sacuvaj} disabled={saving}
          ...
          {saving ? t.cuvanje : (theme.jezik === 'sr' ? 'Sačuvaj Profil →' : 'Save Profile →')}
        </button>
      </div>
    </div>
  );
