...
import { Shield, LogOut, Trash2, Calendar, TrendingUp, Scissors } from 'lucide-react';
...
function Admin() {
  ...
  const handleBrisiSalon = async (id, naziv) => {
    if (window.confirm(`${t.obrisati} "${naziv}"?`)) {
  ...
  const handleBrisiTermin = async (id) => {
    if (window.confirm(sr ? 'Obrisati ovaj termin?' : 'Delete this appointment?')) {
    ...
    return (
      <div style={{ maxWidth: 480, margin: '0 auto', minHeight: '100vh', backgroundColor: '#f8fafc', fontFamily: 'sans-serif', display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: 24 }}>
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <h1 style={{ fontSize: 24, fontWeight: 'bold', color: '#1e293b', margin: 0 }}>{t.adminPanel}</h1>
          <p style={{ color: '#64748b', marginTop: 8 }}>{t.adminOpis}</p>
        </div>
        <div style={{ backgroundColor: 'white', borderRadius: 16, padding: 24, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
          <label style={{ fontSize: 13, fontWeight: 'bold', color: '#475569', display: 'block', marginBottom: 8 }}>{t.adminLozinka}</label>
          <input type="password" placeholder={t.unesiteLozinku} value={lozinka}
      ...
      <div style={{ background: 'linear-gradient(135deg, #1e3a8a, #2563eb)', padding: '20px 20px 28px', borderBottomLeftRadius: 24, borderBottomRightRadius: 24 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ color: 'white', fontWeight: 'bold', fontSize: 18 }}>{t.adminPanel}</span>
          </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
          <div style={{ backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: '12px 10px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#93c5fd', fontSize: 11, margin: 0 }}>{sr ? 'Salona' : 'Salons'}</p>
          </div>
          <div style={{ backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: '12px 10px', textAlign: 'center' }}>
            ...
            <p style={{ color: '#93c5fd', fontSize: 11, margin: 0 }}>{sr ? 'Termina' : 'Appointments'}</p>
          </div>
          <div style={{ backgroundColor: 'rgba(255,255,255,0.15)', borderRadius: 12, padding: '12px 10px', textAlign: 'center' }}>
      ...
      <div style={{ padding: 20 }}>
        <div style={{ display: 'flex', backgroundColor: 'white', borderRadius: 12, padding: 4, marginBottom: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
          <button onClick={() => setAktivnaTabela('saloni')}
            ...
            💈 {sr ? 'Saloni' : 'Salons'} ({saloni.length})
          </button>
          <button onClick={() => setAktivnaTabela('termini')}
            ...
            📅 {sr ? 'Termini' : 'Appointments'} ({termini.length})
          </button>
        </div>

        {loading ? (
          <p style={{ textAlign: 'center', color: '#64748b', padding: 40 }}>{t.ucitavanje}</p>
        ) : aktivnaTabela === 'saloni' ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {saloni.length === 0 ? (
              <p style={{ textAlign: 'center', color: '#64748b', padding: 40 }}>{t.nemaRegistrovanih}</p>
            ) : saloni.map(salon => (
              <div key={salon.id} style={{ backgroundColor: 'white', borderRadius: 14, padding: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.06)' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      ...
                      {salon.verifikovan && (
                        <span style={{ backgroundColor: '#dcfce7', color: '#16a34a', fontSize: 10, fontWeight: 'bold', padding: '2px 6px', borderRadius: 20 }}>✓ {sr ? 'Verifikovan' : 'Verified'}</span>
                      )}
                    </div>
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
                  <button onClick={() => handleVerifikuj(salon.id, salon.verifikovan)}
                    ...
                    {salon.verifikovan ? t.ukloniVerifikaciju : t.verifikuj}
                  </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {termini.length === 0 ? (
              <p style={{ textAlign: 'center', color: '#64748b', padding: 40 }}>{t.nemaTermina}</p>
            ) : termini.map(termin => (
